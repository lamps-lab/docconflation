...
def main(count):
    ...
    for i in cases:
        ...
        cur = f"SELECT database_id, corpus_id, key1, key2, key3, key4, year FROM  FourWordsKey WHERE corpus_id = {i}"
        r.execute(cur)
        ...
        file2 = open("time4.txt","a")
        file2.write(f'{start-end}')
             
def csv_w(Ocorpus, Dcourpus):
    #oringinal_corpus = Ocorpus.replace('/n', '')
    with open('Results/5WordsTrueDuplicates.csv', mode='a') as csv_file:
...
def csv_header():
    with open('Results/5WordsTrueDuplicates.csv', mode='w') as csv_file:
        fieldNames = ['Corpus' , 'Amount' , 'Duplicates']
        writer = csv.DictWriter(csv_file, fieldnames=fieldNames)
        writer.writeheader()       
