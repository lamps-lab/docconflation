...
def main():
    ...
    for i in cases:
        ...
        # merged should contain unique corpus id's 
        print("unique", merged)
...
def keyString(key, year):
    ...
    if(isinstance(year, int)):
        # key string is corpus id's
        key_string = f'SELECT corpus_id FROM FiveWordsKey WHERE (year = "{year}" OR year = "{year+1}" OR year = "{year+2}" OR year = "{year-2}" OR year = "{year-1}") AND (key1 = "{key}" OR key2 = "{key}" OR key3 = "{key}" OR key4 = "{key}")'
        #print(key_string)
    else:
